...
{
  std::cout << "PLANNING ROUTE" << std::endl;
  ...
  auto nearest_start_lanelets = osm_map->laneletLayer.nearest(start, 2);
  if (nearest_start_lanelets.empty()) {
    std::cerr << "Couldn't find nearest lanelet to start." << std::endl;
  } else {
    for (const auto & lanelet : nearest_start_lanelets) {
  ...
  // } else {
  //   // Two nearest lanelets should, in theory, be the lanelets for each direction
  auto nearest_end_lanelets = osm_map->laneletLayer.nearest(end, 2);
  if (nearest_end_lanelets.empty()) {
    std::cerr << "Couldn't find nearest lanelet to goal." << std::endl;
  } else {
    for (const auto & lanelet : nearest_end_lanelets) {
