...
def generate_launch_description():
    ...
    declare_map_yaml_cmd = DeclareLaunchArgument(
        'map', default_value=os.path.join(navigator_dir,'src','planning','nav2_path_planner','nav2_path_planner','nav2_map.param.yaml'), 
        description='Full path to map yaml file to load'
    )

    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(navigator_dir,'param','planning','nav2.param.yaml'),
    ...
    load_nodes = GroupAction(
        [
            ...
            Node(
                ...
                output='screen',
                arguments=['--ros-args', '--log-level', log_level],
                parameters=[{'autostart': autostart}, {'node_names': ['map_server','smoother_server','planner_server']}],
            ),
            ...
            Node(
                ...
                arguments = ['--x', '0', '--y', '0', '--z', '0', '--yaw', '0', '--pitch', '0', '--roll', '0', '--frame-id', '%s_map' % default_namespace, '--child-frame-id', '%s_odom' % default_namespace]
            ),
            Node(
                package='tf2_ros',
                executable='static_transform_publisher',
                arguments = ['--x', '0', '--y', '0', '--z', '0', '--yaw', '0', '--pitch', '0', '--roll', '0', '--frame-id', '%s_odom' % default_namespace, '--child-frame-id', '%s_base_link' % default_namespace]
            ),
        ]
    )
