...
class CurbLocalizer : public rclcpp::Node
{
	public:
		CurbLocalizer()
		: Node("curb_localizer")
		{
			right_curb_sub_ = this->create_subscription<PointCloud2>("/curb_points/right", 1, //Only one message, too slow for queue!
			...
			timer_ = this->create_wall_timer(std::chrono::duration<double>(nudge_period_), std::bind(&CurbLocalizer::tryNDT, this));
	...
	private:
		...
		rclcpp::Subscription<PointCloud2>::SharedPtr right_curb_sub_;
...
int main(int argc, char * argv[])
{
	...
	rclcpp::spin(std::make_shared<CurbLocalizer>());
	rclcpp::shutdown();
	return 0;
}