...
class FakeImuFromPose(Node):

    def __init__(self):
        super().__init__('fake_imu_from_pose')
        ...
        self.pose_sub = self.create_subscription(
            PoseWithCovarianceStamped, '/sensors/zed/pose', self.pose_cb, 10
        )
    ...
    def pose_cb(self, msg: PoseWithCovarianceStamped):
        # Angular vel = (current_orientation-prev_orientation)/dt

        # current vel = (current_pos-prev_pos)/dt

        # Linear acc = current_vel-prev_vel
...
def main(args=None):
    ...
    fake_imu_from_pose = FakeImuFromPose()

    rclpy.spin(fake_imu_from_pose)
    # outfile.close()

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    fake_imu_from_pose.destroy_node()
    rclpy.shutdown()
