...
class ObstaclePeddler(Node):
    def __init__(self):
        ...
        self.throttle_command_pub = self.create_publisher(Float32, '/command/throttle', 10)
    ...
    def front_lidar_cb(self, msg: PointCloud2):
        if not self.tf_buffer.can_transform('base_link', 'lidar_front', self.get_clock().now()):
            ...
            dist_from_front = closest_x - 4.3
            self.get_logger().info("{}".format(dist_from_front))

            throttle_comand = Float32()
            if dist_from_front < self.TARGET_DISTANCE:
                throttle_comand.data = 0.0
            else:
                throttle_comand.data = self.BANG_ON_VALUE
            self.throttle_command_pub.publish(throttle_comand)
