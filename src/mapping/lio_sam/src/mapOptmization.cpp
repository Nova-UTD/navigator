...
#include <gtsam/geometry/Rot3.h> // GTSAM is a library for factor graphs and Bayes optimization. Used for loop closure, GPS correction, etc.
...
{
    PCL_ADD_POINT4D // Do we need a semicolon here?
    PCL_ADD_INTENSITY;                  // preferred way of adding a XYZ+padding... what???
    float roll;
    float pitch;
    float yaw;
    double time;
...
public:
    ...
    // gtsam
    NonlinearFactorGraph gtSAMgraph; // Our factor graph https://gtsam.org/tutorials/intro.html#magicparlabel-65411
    ...
    vector<pcl::PointCloud<PointType>::Ptr> surfCloudKeyFrames; // Pcloud for surface features
    ...
    void addOdomFactor()
    {
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            noiseModel::Diagonal::shared_ptr odometryNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-7, 1e-7, 5e-5, 1e-2, 1e-2, 5e-2).finished());
