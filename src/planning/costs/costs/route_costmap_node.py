...
class RouteCostmapNode(Node):
    ...
    def __init__(self):
        ...
        self.goal_marker_pub = self.create_publisher(Marker, "/planning/goal_marker", 1)

        route_sub = self.create_subscription(Path, "/planning/route", self.routeCb, 1)
        self.route = None
    ...
    def buildRouteCostmap(self):
        ...
        try:
            ...
            ego_tf = self.tf_buffer.lookup_transform(
                "base_link", "map", rclpy.time.Time(), rclpy.time.Duration(seconds=5.0)
            )

            roll, pitch, yaw = euler_from_quaternion(
                quat_to_numpy(ego_tf.transform.rotation)
            )
            ...
            for i, pose in enumerate(self.route_remaining):
                x = (
                    pose.pose.position.x * np.cos(yaw)
                    - pose.pose.position.y * np.sin(yaw)
                    + ego_tf.transform.translation.x
                )
                y = (
                    pose.pose.position.x * np.sin(yaw)
                    + pose.pose.position.y * np.cos(yaw)
                    + ego_tf.transform.translation.y
                )
            ...
            if len(keep_idxs) == 0:
                self.get_logger().warning(
                    "Did not find any route points ahead of the vehicle."
                )
            ...
            for i in range(1, len(route_baselink_x)):
                ...
                steps = int(
                    np.ceil(max(abs(dx), abs(dy)) / gridres)
                )  # find the axis that changes the most
            ...
            pixel_steps = [
                ...
                3,
                4,
            ]  # how many costmap cells are painted to either "side" according to the cost_gradient below
                    ...
                    if jold < j:  # last point is behind current one
                        for d in pixel_steps:
                            try:  # all the try/excepts are to catch when i+d, i-d, j+d, j-d go outside the bounds of the costmap
                                routemap[i - d, j] = min(
                                    routemap[i - d, j], cost_gradient[d - 1]
                                )
                                routemap[i + d, j] = min(
                                    routemap[i + d, j], cost_gradient[d - 1]
                                )
                        ...
                        if iold == i:  # route is heading straight forward
                            ...
                            for d in pixel_steps:
                                try:
                                    routemap[i - d, j + d] = min(
                                        routemap[i - d, j + d], cost_gradient[d - 1]
                                    )
                                    routemap[i + d, j - d] = min(
                                        routemap[i + d, j - d], cost_gradient[d - 1]
                                    )
                                    routemap[i - d, j + d - 1] = min(
                                        routemap[i - d, j + d - 1], cost_gradient[d - 1]
                                    )
                                    routemap[i + d - 1, j - d] = min(
                                        routemap[i + d - 1, j - d], cost_gradient[d - 1]
                                    )
                                    routemap[i - d + 1, j + d] = min(
                                        routemap[i - d + 1, j + d], cost_gradient[d - 1]
                                    )
                        ...
                        for d in pixel_steps:
                            try:
                                routemap[i, j - d] = min(
                                    routemap[i, j - d], cost_gradient[d - 1]
                                )
                                routemap[i, j + d] = min(
                                    routemap[i, j + d], cost_gradient[d - 1]
                                )
            ...
            ExtrapolationException,
            ConnectivityException,
        ) as e:  # typically get some errors on startup as the tf buffer fills
            self.get_logger().warning(
                "!!! Error finding transform to build route grid !!!"
            )
    ...
    def publish(self, routemap, goal):
        ...
        route_cost_msg.info.origin.position.x = (
            -20.0
        )  # TODO: should avoid hard coding this
        route_cost_msg.info.origin.position.y = (
            -30.0
        )  # TODO: should avoid hard coding this
    ...
    def make_waypoint_costmap(self, waypoint):
        ...
        for i in range(costmap.shape[0]):
            for j in range(costmap.shape[1]):
                ...
                dist = np.sqrt(
                    (x - waypoint.position.x) ** 2 + (y - waypoint.position.y) ** 2
                )
