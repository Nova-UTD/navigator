...
class DDRNetInferenceNode(Node):
    ...
    def __init__(self, weights_file):
        ...
        self.sync = message_filters.ApproximateTimeSynchronizer([rgb_sub, depth_sub], 1, 0.5)
    ...
    def rgbd_cb(self, rgb_msg: Image, depth_msg: Image):
        ...
        image = cv2.resize(color_image, (color_image.shape[1]*2, color_image.shape[0]*2), interpolation=cv2.INTER_LINEAR)
        ...
        road_image[seg_map == 0 ] = 255     # for all classified pixels that are road (0), mark them as white (255)
        ...
        depth_image = cv2.resize(depth_image, (image_size_x, image_size_y), interpolation=cv2.INTER_AREA)
        ...
        road_pcd = road_pcd[road_pcd[:, -1] < 0.2]  # remove points that are above 0.2 m on z-axis

        # turn into pointcloud msg
        road_pcd.dtype = [('x', np.float32), ('y', np.float32), ('z', np.float32)]  # specify fields
