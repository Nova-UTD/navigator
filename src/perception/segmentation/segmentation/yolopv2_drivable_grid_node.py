...
Output is strict binary, matching the rest of this project's grids: 0 =
confirmed drivable (free, rendered white in RViz), 100 = not (occupied/
unknown). No temporal history, no LiDAR validation, no vehicle-footprint
prior -- same "prove the base projection first" philosophy as the
lane-line version's Step 1, applied to this new source.
...
from segmentation.bev_geometry import CAMERAS, CamLUT, GRID_SIZE, RESOLUTION, ORIGIN_X, ORIGIN_Y
...
class Yolopv2DrivableGridNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'Yolopv2DrivableGridNode ready (drivable-area head projection) — {_OUTPUT_TOPIC}')
    ...
    def _publish_loop(self):
        ...
        grid_out = np.where(confirmed, 0, 100).astype(np.int8)
