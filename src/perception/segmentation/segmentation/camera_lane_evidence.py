...
so this module detects it there instead — restricted to the road-classified
region (PSPNet's /semantic/* output), using a morphological top-hat filter
rather than a flat/global brightness threshold. Top-hat isolates features
that are both (a) thinner than the structuring element and (b) locally
brighter than their surroundings, which is structurally what a lane line is
— it stays robust to slow lighting/shadow gradients across the frame,
unlike a single global or even a naive per-frame-percentile threshold
(the flaw that made the LiDAR version fail: "top 15% of this frame" is
always non-empty even when there's no real signal, since it's relative to
the frame's own noise floor rather than any structural property).
...
ROAD_COLOR = (128, 64, 128)  # matches image_segmentation_node._PALETTE[0]
...
def marking_candidate_mask(rgb_image, road_mask,
                            kernel=TOPHAT_KERNEL, tophat_thresh=TOPHAT_THRESHOLD):
    """rgb_image: HxWx3 uint8 raw camera frame. road_mask: HxW bool.
    ...
    AND within the road-classified region.
    """
    gray = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2GRAY)
    tophat = cv2.morphologyEx(gray, cv2.MORPH_TOPHAT, kernel)
    bright = tophat >= tophat_thresh
    return bright & road_mask
